...
try: 
    from Transformer_modules import LayerNorm, EncoderLayer, DecoderLayer, MultiHeadedAttention, PositionwiseFeedForward, LearnablePositionalEncoding, CLIP_ff, NoamOpt, run_epoch, Variable, DataGenerator, LearnableClassEmbedding, PositionalEncoding, EncoderDecoder, Encoder, Decoder, Embeddings, Generator
except ModuleNotFoundError:
    from analysis.raman.models.Transformer_modules import LayerNorm, EncoderLayer, DecoderLayer, MultiHeadedAttention, PositionwiseFeedForward, LearnablePositionalEncoding, CLIP_ff, NoamOpt, run_epoch, Variable, DataGenerator, LearnableClassEmbedding, PositionalEncoding, EncoderDecoder, Encoder, Decoder, Embeddings, Generator
import torch
...
def make_model(tgt_vocab, src_vocab=1024, depth=6, d_model=512, d_ff=1024, h=8, dropout=0.1, mode='train'):
    ...
    cls_embedding = LearnableClassEmbedding(d_model, dropout) if 'retrieval' in mode else nn.Identity()
    src_length = position(cls_embedding(spec_encoding(torch.randn([1,1,src_vocab])))).shape[-2]
    src_embed=nn.Sequential(c(spec_encoding), 
                            c(cls_embedding), 
                            c(position))
    # if 'retrieval' in mode or 'pretrain' in mode:
    tgt_emb = nn.ModuleList([c(mol_embedding), 
                             c(cls_embedding), 
                             c(position)])
    # else:
    ...
    tgt_ff_norm = CLIP_ff(d_model, True) if 'retrieval' in mode else nn.Identity()
    src_ff_norm = CLIP_ff(d_model, True) if 'retrieval' in mode else nn.Identity()
    model = EncoderDecoder(
        encoder=Encoder(EncoderLayer(d_model, c(attn), c(ff), dropout), depth),
        decoder=Decoder(DecoderLayer(d_model, c(attn), c(attn),
                                     c(ff), dropout, mode), depth),
        src_embed=src_embed,
        tgt_embed=tgt_emb,
        generator=Generator(d_model, tgt_vocab),
        tgt_ff_norm=tgt_ff_norm,
        src_ff_norm=src_ff_norm,
        mode=mode)
