...
class ResNet(nn.Module):
    def __init__(self, hidden_sizes, num_blocks, input_dim, in_channels, n_classes=957, fc_num_layers=0, 
                 fc_dim=1024, use_mixer=0, mixer_num_layers=0, use_se=1, use_res=1):
        ...
        self.spec_emb = nn.Sequential(
                    nn.Conv1d(1, self.in_channels, kernel_size=16, stride=16),
...
def resnet(depth=6, hidden_size=1024, block_size=1,  input_dim=1024,
                 in_channels=256, n_classes=37, fc_num_layers=0, fc_dim=1024, use_mixer=1, mixer_num_layers=1,
                 use_se=1, use_res=1):
    ...
    config = {'hidden_sizes':hidden_sizes, 'num_blocks': num_blocks, 'input_dim': input_dim, 'in_channels': in_channels, 
              'n_classes': n_classes, 'fc_num_layers': fc_num_layers, 'fc_dim': fc_dim, 'use_mixer': use_mixer, 'mixer_num_layers':mixer_num_layers, 'use_se':use_se, 'use_res':use_res}
