...
class AcceptanceTypeConfig(BaseModel):
    ...
    execution_mode: str = Field(default="local", description="执行模式。")
...
class AcceptanceRunItem(BaseModel):
    ...
    sample_execution_id: str = Field(description="批次内样本执行 ID。")
    ...
    text_report: str = Field(default="", description="样本文本报告。")
    artifacts: list[TaskArtifactItem] = Field(default_factory=list, description="样本产物列表。")
